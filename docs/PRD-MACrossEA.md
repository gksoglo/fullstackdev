...
All signal calculations (Modules A, B, C) use shift ≥ 1 only. No module ever reads shift 0.

**Per-module application:**

- **Module A (LTF):** the crossover is detected between shift 2 and shift 1 — i.e., `FastMA[2] ≤ SlowMA[2]` and `FastMA[1] > SlowMA[1]` (bullish case; mirrored for bearish). Shift 1 is the signal bar. `TrendConfirm_Bars = 3` means `MA[3] < MA[2] < MA[1]` (bullish) evaluated on both fast and slow MA — the signal bar (shift 1) is included in the count.
- **Module B (HTF):** uses the most recently completed HTF candle at the moment the LTF signal bar closes — this is HTF shift 1, never HTF shift 0, even if the HTF candle has been forming for a long time relative to the LTF. `HTF_TrendConfirm_Bars = 5` means `HTF_MA[5] < HTF_MA[4] < HTF_MA[3] < HTF_MA[2] < HTF_MA[1]` (uptrend case).
...
| TrendConfirm_Bars | int | 3 | Number of consecutive closed bars (including the signal bar, shift 1) over which fast & slow MA must move monotonically in cross direction. Intentionally independent of Regression_Bars below — see note under input 6. |
...
| HTF_TrendConfirm_Bars | int | 5 | Number of consecutive closed HTF MA values (HTF shift 1 .. shift HTF_TrendConfirm_Bars) required to establish trend |
...
Using HTF shift 1 through shift HTF_TrendConfirm_Bars (see Section 4.0 for the synchronization rule — always the most recently completed HTF candle, never the forming one): if strictly decreasing → HTF is in downtrend (`HTF_MA[5] > HTF_MA[4] > HTF_MA[3] > HTF_MA[2] > HTF_MA[1]` for the default of 5 → only SHORT signals permitted); if strictly increasing → uptrend (only LONG signals permitted). This is a strict monotonic regime classification, not a general "is it trending" test — a single unchanged or countertrend value anywhere in the window (e.g. 100, 101, 102, 102, 103) causes the whole window to fail and fall through to ranging/blocked below. Combined with Module A's own strict-monotonic check and the other filters, the overall strategy can end up considerably more selective than "trend filter" might suggest — see the veto-rate testing note in Section 6.

If neither monotonic condition holds → HTF is ranging/neutral. Fixed rule: all trades are blocked while ranging, regardless of Module A or C.
